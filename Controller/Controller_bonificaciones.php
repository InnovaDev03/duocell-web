...
include('../Model/Model_bonificaciones.php');
...
function registro_labor()
{
	...
	$data["txt_nivel1"]     	= $_POST['txt_nivel2'];
...
{ 
    ...
    foreach ($movements as $data) {
        ...
        $dato['id_presupuesto']  =$data['id_bonificacion'];
        ...
        $dato['nombre']  =$data['item'];
        
		
		if ($data['estado'] == 1){
			
			$dato['button'] = '  <button type="button" id_labor="'.$data['id_bonificacion'].'"  enero="'.$data['enero'].'" 
            febrero="'.$data['febrero'].'"  marzo="'.$data['marzo'].'"  abril="'.$data['abril'].'" 
            mayo="'.$data['mayo'].'"  junio="'.$data['junio'].'"  julio="'.$data['julio'].'" 
            agosto="'.$data['agosto'].'"  septiembre="'.$data['septiembre'].'"  octubre="'.$data['octubre'].'" 
            noviembre="'.$data['noviembre'].'"  diciembre="'.$data['diciembre'].'"  nombre="'.$data['item'].'"  
            class="btn btn-xs btn-warning btn_load_edit_labor" ><acronym title="Editar!" lang="es"><i class="far fa-edit"></i></acronym></button>
<button type="button"  id_labor="'.$data['id_bonificacion'].'"  nombre="'.$data['item'].'"   class="btn btn-xs btn-danger btn_delete_labor"><acronym title="Eliminar !" lang="es"><i class="fa fa-trash" aria-hidden="true"></i></acronym></button>

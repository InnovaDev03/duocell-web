...
function vaciar_venta()
{
    ...
    $movements  = $Consult->vaciar_venta($data);
...
function serarch_productos()
{
    ...
    foreach ($movements as $movement) {
        ...
        $dato["id"]                       = $movement["id"];
        $dato["categoria"]                = $movement["categoria"];
        $data[]=$dato;
    }
...
function agregar_item()
{
	...
	$ata["txt_precio_oculto"] = $_POST['txt_precio_oculto'];
...
{ 
    ...
    foreach ($movements as $data) {
        ...
        $i++;
        $dato['id']            = $i.' - <button type="button" pr_item ="'.$data['item'].'" pr_id ="'.$data['id'].'"    class="btn btn-xs btn-danger btn_delete_item"><acronym title="Borrar item!" lang="es"><i class="fa fa-trash" aria-hidden="true"></i></acronym></button>';
...
function registrar_venta()
{
    ...
    $data["txt_cliente"]           = $_POST['txt_cliente2'];
    ...
    if(isset($_POST['txt_dias_credito']))
    {
        if($_POST['txt_pago'] == 'CONTADO')
        {
            $data["txt_dias_credito"]            = 0;
        }   
        else
        {
            $data["txt_dias_credito"]            = $_POST['txt_dias_credito'];
        } 
    }
    else
    {
...
function serarch_stockproductos()
{
    ...
    $Consult    = new ModelPg();
    $movements  = $Consult->serarch_stockproductos($_GET['term'],$_GET['txt_bodega']);
    foreach ($movements as $movement) {
        $dato["text"]                     = $movement["text"];
        $dato["id"]                       = $movement["id"];
        $data[]=$dato;
...
function buscar_precio()
{
 ...
 $data['txt_cantidad']=$_REQUEST["txt_cantidad"];
...
function consultar_vendedorsi()
{
    ...
    $data["txt_id"]          = $_POST['txt_id'];
    $data['iduser']=$_SESSION["gb_id_user"];
    $data['perfil']= $_SESSION["gb_perfil"];
    $i                            = 1;
    ...
    $movements  = $Consult->consultar_vendedorsi($data);
    foreach ($movements as $movement) {

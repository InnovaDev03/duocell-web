...
include('../Model/Model_mer_registro.php');
...
function serarch_cliente()
{
    ...
    $Consult    = new ModelPg();
    $movements  = $Consult->serarch_cliente($_GET['term'],$_SESSION["gb_id_user"]);
    foreach ($movements as $movement) {
        $dato["text"]                     = $movement["text"];
        $dato["id"]                       = $movement["id"];
        $dato["canal"]                       = $movement["canal"];
        $dato["subcanal"]                       = $movement["subcanal"];
        $dato["canalnombre"]                       = $movement["canalnombre"];
        $dato["subcanalnombre"]                       = $movement["subcanalnombre"];
        $dato["codigocliente"]                       = $movement["codigocliente"];
        $dato["codigovendedor"]                       = $movement["codigovendedor"];
    
    
    if($movement["cupo_disponible"] == null)
    {
     $dato["cupo_disponible"]                       = 0;
    }
    else
    {
     $dato["cupo_disponible"]                       = $movement["cupo_disponible"];
    }
    
     if($movement["saldo_vencido"] == null)
    {
     $dato["saldo_vencido"]                       = 0;
    }
    else
    {
     $dato["saldo_vencido"]                       = $movement["saldo_vencido"];
    }

    if($movement["dias"] == null)
    {
     $dato["dias_credito"]                       = 0;
    }
    else
    {
     $dato["dias_credito"]                       = $movement["dias"];
    }

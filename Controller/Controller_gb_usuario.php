...
include('../Model/Model_gb_usuario.php');
...
{ 
    ...
    foreach ($movements as $data) {
        ...
        $dato['button']        =  '
                                  <button type="button" dp_id="'.$data['gb_id'].'" dp_nombre="'.$data['gb_nombre'].'" dp_usuario="'.$data['gb_usuario'].'" dp_email="'.$data['gb_email'].'" 
                                  telefono="'.$data['telefono'].'" cod_vendedor="'.$data['cod_vendedor'].'"  
                                  dp_perfil="'.$data['dp_id_perfil'].'" dp_clave="'.$data['gb_clave'].'"  class="btn btn-xs btn-warning btn_load_edit_user"><acronym title="Editar usuario!" lang="es"><i class="far fa-edit"></i></acronym></button>

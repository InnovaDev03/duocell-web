...
=============================================*/
    public function secuencia_codigo($txt_user){
        ...
        $sql         = "SELECT  count(*) AS secuencia  FROM pr_venta";
...
public function agregar_item($data)
{
    ...
    $mysqli->autocommit(FALSE);
                ...
                if($resut == 0)
                {
                    ...
                    $sql_insert = "INSERT INTO pr_venta_deta_templle 
                    ( 	
                        pr_item,
                        pr_descripcion,
                        pr_cantidad,
                ...
                {   
                    ...
                    $sql = "SELECT * FROM  pr_venta_deta_templle WHERE pr_item ='".$data['txt_id'] ."' AND pr_usuario='".$data['txt_user'] ."'";
                    $resultado = $mysqli->query($sql);
                    $fila = $resultado->fetch_assoc();
                    $total = ($fila['pr_cantidad'] + 1) *  $data['txt_precio'];
                    $sql_insert = "UPDATE pr_venta_deta_templle SET pr_cantidad=pr_cantidad+1,pr_total='".$total."',pr_precio='".$data['txt_precio']."' WHERE pr_usuario='".$data['txt_user'] ."'";
                }
...
public function table_venta($txt_id)
{
    $mysqli      = conexionMySQL();
    $data        = array();
    $sql         = "SELECT * FROM pr_venta_deta_templle WHERE pr_usuario='".$txt_id."'";
    $resultado   = $mysqli->query($sql);
    while ($fila = $resultado->fetch_assoc()) {
...
public function eliminar_item($data)
{
    ...
    $sql_insert = "DELETE FROM pr_venta_deta_templle WHERE pr_id ='".$data['txt_id']."'";
    $resultado  = $mysqli->query($sql_insert);
    ...
    date_default_timezone_set('America/Guayaquil');
                   ...
                   // $resut = $this->validar_cantidad_imei($data['txt_user']);
                    ...
                    {   
                            ...
                            $sql_insert = "INSERT INTO pr_venta 
                            ( 	
                                pr_id_cadena,
                                pr_id_tienda,
                                pr_codigo,
                            ...
                            $rp_detalles = "SELECT * FROM pr_venta_deta_templle WHERE pr_usuario='".$data['txt_user']."'" ;
                            ...
                            if($n>0)
                            {
                                ...
                                {
                                    $sql_insert = "INSERT INTO pr_venta_detalle 
                                    ( 	
                                        pr_id_venta,
                                        pr_item,
                                        pr_descripcion,
                            ...
                            /** VACIAMOS TABLAS**/
                            $sql_insert = "DELETE FROM pr_venta_deta_templle WHERE pr_usuario='".$data['txt_user']."'";
                            $resultado  = $mysqli->query($sql_insert);
                            if ($resultado) 
                            {
                            ...
                            $data['dp_modulo']          = 'PROMOTORIA->CREAR VENTA';
...
private function validar_cantidad_imei($txt_user) 
{
    ...
    $total_item = 0;
    $sql = "SELECT SUM(pr_cantidad) as total FROM pr_venta_deta_templle WHERE pr_usuario='$txt_user'";
    $resultado = $mysqli->query($sql);
    while ($fila = $resultado->fetch_assoc()) {
...
{
    $mysqli = conexionMySQL();
    $sql = "SELECT * FROM  pr_venta_deta_templle WHERE pr_item ='$txt_id' AND pr_usuario='$txt_user'";
...
private function total_venta($id_usuario)
{
    ...
    $sql       = "SELECT SUM(pr_total) AS total FROM pr_venta_deta_templle WHERE pr_usuario='".$id_usuario."'";
    $resultado = $mysqli->query($sql);

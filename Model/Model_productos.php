...
public function registrar_cadena($data)
{
    ...
    $mysqli->autocommit(FALSE);
        ...
        if($resut == 0)
        {
                    ...
                    $sql_insert = "INSERT INTO productos_habilitados 
                    ( 	
                        nombre,
                        codigo,
                        producto,
                    ...
                    (
                        ...
                        '" . $data['txt_codigoproducto'] . "',
                        '" . $data['txt_nombreproducto'] . "',
                        '" . $data['txt_user'] . "',
                        '1'
                    ) ";
            ...
            if ($resultado) {
                ...
                /** DATOS BITACORA **/
                $data['dp_modulo']          = 'HABILITACIÓN->PRODUCTO';
                $data['dp_accion']         =  "INSERTAR";
                $data['dp_descripcion']     = 'HABILITAR PRODUCTO : '.$data['txt_nombre_cadena'];
        ...
        {
            $Global->salir_json(2,"Producto habilitado! ");
        }
...
public function table_cadenas()
{
    $mysqli      = conexionMySQL();
    $data        = array();
    $sql         = "SELECT * FROM productos_habilitados where estatus=1";
...
public function editar_cadena($data)
{
    ...
    $mysqli->autocommit(FALSE);
        ...
        if($resut == 0)
        {
            ...
            $sql_insert = "UPDATEx gamas SET nombre='".$data['txt_nombre_cadena']."',GAMA='".$data['txt_ciudad_cadena']."' WHERE id='".$data['txt_id_cadena']."'  ";
            $resultado  = $mysqli->query($sql_insert);
            if ($resultado) {
...
public function disabled_cadena($data)
{
    ...
    $mysqli->autocommit(FALSE);
            ...
            $sql_insert = "UPDATE productos_habilitados SET estatus='0' WHERE id='".$data['txt_id']."'  ";
            $resultado  = $mysqli->query($sql_insert);
            if ($resultado) {
                ...
                /** DATOS BITACORA **/
                $data['dp_modulo']          = 'DESACTIVAR->PRODUCTO';
                $data['dp_accion']         =  "DESACTIVAR";
                $data['dp_descripcion']     = 'DESACTIVAR PRODUCTO : '.$data['txt_nombre'];
                $data['dp_sql']             = $sql_insert;
                $data['dp_user']            = $data['txt_user'];
                $data['dp_fecha_registro']  = date('Y-m-d');
                $data['dp_hora_registro']   = date("g:i:s-a");

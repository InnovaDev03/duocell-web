...
public function registro_labor($data)
{
    ...
    $mysqli->autocommit(FALSE);
        ...
        if($resut == 0)
        {
                    ...
                    $sql_insert = "INSERT INTO `presupuesto`( `anio`, `rubro`, `nivel1`,
                     `nivel2`, `nivel3`, `enero`, `febrero`, `marzo`, `abril`, `mayo`, 
                     `junio`, `julio`, `agosto`, `septiembre`, `octubre`, `noviembre`,
                      `diciembre`, `fecha_creacion`, `estado`,id_cadena, id_tienda, `id_usuariofk`)				
                    VALUES
                    (
                        '" . $data['txt_ano'] . "',
						'" . $data['labor'] . "',
            ...
            if ($resultado) {
                ...
                /** DATOS BITACORA **/
                $data['dp_modulo']          = 'PRESUPUESTO->REGISTRO';
                $data['dp_accion']         =  "INSERTAR";
                $data['dp_descripcion']     = 'CREAR PRESUPUESTO : '.$data['txt_ano'] . $data['labor'] ;
        ...
        {
            $Global->salir_json(2,"Presupuesto ya registrada! ");
        }
...
public function eliminar_labor($data)
{
    ...
    $mysqli->autocommit(FALSE);
            ...
            $sql_insert      = "UPDATE presupuesto SET estado = 0  WHERE id_presupuesto ='".$data["txt_id"]."'  ";
            $resultado       = $mysqli->query($sql_insert);
            if ($resultado) {

                /** DATOS BITACORA **/
                $data['dp_modulo']          = 'PRESUPUESTOS->ELIMINACION';
                $data['dp_accion']          = "ELIMINAR";
                $data['dp_descripcion']     = 'ELIMINAR PRESUPUESTO ID : '.$data['txt_id'];
                $data['dp_sql']             = $sql_insert;
                $data['dp_user']            = $data['txt_user'];
                $data['dp_fecha_registro']  = date('Y-m-d');
                $data['dp_hora_registro']   = date("g:i:s-a");
    ...
    $mysqli->autocommit(FALSE);
       ...
       // $resut = $this->validar_labor_edit($data['txt_id'],$data['txt_labor_e']);
        ...
        {     
            ...
            $sql_insert = "UPDATE  presupuesto SET
            ...
            `id_usuariofk`='".$data['txt_user']."' WHERE id_presupuesto='".$data['txt_id']."' "; 
            $resultado  = $mysqli->query($sql_insert);
            $id_usuario =  $mysqli->insert_id;
            if ($resultado) {

                /** DATOS BITACORA **/
                $data['dp_modulo']          = 'PRESUPUESTO->EDICION';
        ...
        {
            $Global->salir_json(2,"Presupuesto editado! ");
        }
...
if($resut==1){
    ...
    {
        $mysqli = conexionMySQL();
        $sql = "SELECT * FROM  presupuesto WHERE id_tienda ='$labor' and anio='$datax' and estatus=1 ";
        $resultado = $mysqli->query($sql);
        $n = $resultado->num_rows;
        if ($n > 0) {
            $salida = 1;
...
public function table_ventas($data1,$data2)
{
    $mysqli      = conexionMySQL();
    $data        = array();
    $sql         = "SELECT presupuesto.*, nombre FROM presupuesto 
    inner join pa_tiendas ON
pa_tiendas.id = presupuesto.id_tienda
 where  presupuesto.id_cadena =".$data1." and presupuesto.anio=".$data2." and estado=1  ORDER BY nombre asc ";

...
public function registrar_cadena($data)
{
    ...
    $mysqli->autocommit(FALSE);
        ...
        if($resut == 0)
        {
                    ...
                    $sql_insert = "INSERT INTO pa_cadenas 
                    ( 	
                        nombre,
                        ciudad ,
                        direccion,
            ...
            if ($resultado) {
                ...
                $data['dp_descripcion']     = 'CREAR CADENA : '.$data['txt_nombre_cadena'];
...
public function disabled_cadena($data)
{
    ...
    $mysqli->autocommit(FALSE);
            ...
            $sql_insert = "UPDATE pa_cadenas SET estatus='0' WHERE id='".$data['txt_id']."'  ";
            $resultado  = $mysqli->query($sql_insert);
            if ($resultado) {
                ...
                $data['dp_descripcion']     = 'DESACTIVAR CADENA : '.$data['txt_nombre'];
                $data['dp_sql']             = $sql_insert;
                $data['dp_user']            = $data['txt_user'];
                $data['dp_fecha_registro']  = date('Y-m-d');
                $data['dp_hora_registro']   = date("g:i:s-a");

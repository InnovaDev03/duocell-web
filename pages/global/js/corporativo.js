...
$(document).on("click","#btn_crear_venta",function(){
    ...
    if (txt_cliente ==0) {
        NotifiError("Seleccione cliente!");
        return false;
    }  
    var comprobar = $('#txt_fecha').val().length * $('#txt_pago').val().length;
    if (comprobar > 0) {
    ...
    var value = {
        ...
        txt_cadena        : txt_cliente,
        txt_tienda        : $('#txt_cliente').val(),
...
//guardar precio
    $(document).on("click", ".btn_guardar_precio", function () {

        id_form = $(this).attr("contadores");
        nombreproducto = $(this).attr("nombreproducto");
        
            //document.getElementById("btn_registro_cargo").disabled = true;
            var formulario   = $('#formulario'+id_form);
            var datos = formulario.serialize();
            var archivos = new FormData();
            
            var url = 'upload_price.php';
            

            $.ajax({

                url: url + '?' + datos+'&id_form='+id_form+'&nombreproducto='+nombreproducto,
                type: 'POST',
                contentType: false,
                data: archivos,
                processData: false,
                ...
                success: function(data) {
                   // var data = jQuery.parseJSON(data);
                    ...
                    if (data == 1) {
                        AlertaExito('EXITO', 'EXITO');
                        NotifiExito('Precio registrado!');


/*                        document.getElementById("btn_registro_cargo").disabled = false;
                        AlertaExito('EXITO', 'EXITO');
                        NotifiExito('Trazabilidad agregada');

                        $("#txt_tipo_trazabilidad").val(1);
                        $("#txt_edo_trazabilidad").val(1);
                        $("#txt_descripcion").val('');
                        $('#registrotrazabilidad')[0].reset();
                        
                        var table = $('#tabla_trazado').DataTable(); 
                        table.ajax.reload( null, false );
                    	var table = $('#tabla_operaciones_general').DataTable(); 
                        table.ajax.reload( null, false ); 
                    	
                        $("#modal_trazabilidad").modal("hide");
                        $(".previsualizar_archivo").hide();    
                            
               
                        $("#listado_estados").show();

                        busca_estado($("#id_carga_general").val()); 
...
//editar precio
    $(document).on("click", ".btn_edit_precio", function () {
        ...
        nombreproducto = $(this).attr("nombreproducto");
            ...
            var url = 'upload_price_edit_corporativo.php';
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data == 1) {
                        AlertaExito('EXITO', 'EXITO');
                        NotifiExito('Precios actualizad@s!');

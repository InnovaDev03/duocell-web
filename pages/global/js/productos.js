...
$(document).on("click", "#btn_save_cadena", function () {
    ...
    var comprobar = $('#txt_producto').val().length ;
    if (comprobar > 0) {
        //$("#txt_producto").val()
        var value = {

            txt_nombre_cadena    : txt_producto,
            txt_codigoproducto    : txt_codigoproducto,
            txt_nombreproducto    : txt_nombreproducto,
            txt_option           : '1'
        };
        $.ajax(
            {
                url: "../../Controller/Controller_productos.php",
                type: "POST",
                data: value,
                beforeSend: function () {
...
$(document).on("click", "#btn_editar_cadena", function () {
	...
	$(document).on("click", ".btn_delete_cadena", function () {
		...
		Swal.fire({
			title: "Desea deshabilitar el producto?",
			text: "Producto a deshabilitar: " + $(this).attr("pr_nombre") + "",
			icon: 'warning',
				...
				$.ajax(
					{
						...
						success: function (data, textStatus, jqXHR) {
							...
							if (data.result == 1) {
                                ...
                                table_cadenas(parametros);
								AlertaExito('EXITO', 'EXITO');
								NotifiExito('Producto deshabilitado!');
							}
							else {
								NotifiError(data.error);
								AlertaExito('EXITO', 'EXITO');
...
$("#txt_producto").autocomplete({
    ...
    select: function (event, ui) {
        ...
        if (ui.item.value == "0") {
            ...
            txt_codigoproducto=ui.item.value;
            txt_nombreproducto=ui.item.label;
        }
    },

...
header("Content-Disposition: attachment; filename= precios_".date('Y-m-d g:i:s-a').".xls");
...
$conn      = conexionSQLSI();
 $sql = "SELECT * from precios where estatus=1 AND producto!='-' ";
 $Codigo2 ='';
 $productconcat = '';
 $itemconcat='';
 // echo $sql;
...
$sqlE         = "SELECT Codigo,Descripcion from CCSubCanal where CodCanal='002' and estado='A' AND Codigo='".$subcanal."'";
...
*/
$sql         = "select count(*) total from CCSubCanal where CodCanal='002' and estado='A' ";
  ...
  $sql         = " SELECT Codigo,Descripcion from CCSubCanal where CodCanal='002'  and estado='A'  order by Descripcion desc  ";
  $resultado   = sqlsrv_query($conn, $sql);
  $contador=0;
  $concat='';

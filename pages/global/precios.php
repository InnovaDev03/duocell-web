          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <button type="button" class="pull-right btn btn-default" id="sendEmail">Open Market
                  <i class="fa fa-arrow-circle-right"></i>
              </ol>
            </div>
          </div>
...
<form  style="display: none" action="" method="post" name="frmExcelImport" id="frmExcelImport" enctype="multipart/form-data">
            ...
            <hr>
                ...
                <div class="col-sm-12 col-md-12 col-lg-2">
                  <div class="form-group">
                      <a target="_blank" href="excel.php" class="btn btn-outline-info btn-block btn-flat"><i class="fas fa-download"></i> Descargar datos</a>                    
                  </div>
                </div>
...
$query = "INSERT `precios_historial` 
...
$query = "update `precios` set estatus=0,
          ...
          foreach ($Reader as $Row)
          {
            ...
            if(isset($Row[0])) {
             ...
             $subcanal = "";      
              ...
              $credito = "";
              if(isset($Row[4])) {
                 $credito = mysqli_real_escape_string($con,$Row[5]);
              }
              $redir = '';


              if ($producto=='Producto' or $subcanal=='Subcanal' or $precio=='Mínimo' ){
              } else {
            $contador=$contador+1;    
                $concatsql='';
                $concatval='';
...
$query = "INSERT INTO `precios`(`producto`, `subcanal`,  $concatsql
...
//                        $message = "Excel importado correctamente";
            ...
            {
              echo '<script>alert("Error en el formato");</script>';
            }
       
echo "<script>location.href='?opc=precios&cant=".$contador."'</script>";      
        }
...
<td>
  ...
  $sql         = "  SELECT Descripcion from CCSubCanal where CodCanal='002'  and estado='A' order by Descripcion desc";
...
<!--                      <div class="col-lg-12 col-ms-12 col-xs-12">
 ...
 <?php
    ...
    //$mysqli      = conexionMySQL()
    $data        = array();
    $sql         = "SELECT * FROM precios WHERE producto = '".$nombreproducto. "' and estatus=1 ";
    $resultadoa   = $mysqli->query($sql);
    $contadorproducto=0;
    while ($fila = $resultadoa->fetch_assoc()) {
